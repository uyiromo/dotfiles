...
# 4. merge branch
# 5. restore if needed
# 6. rebase against main

gitcmd "git push -u origin ${branch}"
gitcmd "git stash push -m ${datetime}"
gitcmd "git checkout main"
gitcmd "git merge ${branch} --no-verify --no-ff --log -m \":twisted_rightwards_arrows: Merge branch '${branch}'\""
...
# rebase
gitcmd "git rebase main --no-verify"
